...
export const Products = (
  productsList = [],
  attribute = { isFeatured: false, category: "", directLink: "", withDirectLink: false, withContainer: false, verticalScrollable: false, displayType: "default" }
) => {
  const { isFeatured, category, directLink, withDirectLink, withContainer, verticalScrollable, displayType } = attribute;
        ...
        ? productsList.map((product) => {
            if (displayType === "default") {
              ...
              return createCustomElement(
                "div",
                {
                  class: `product-item ${verticalScrollable ? "product-item-unwrapped" : ""}`,
                  style: `background-image: url("${product.image}");`,
                  id: `${product.id}_${product.title.split(" ").join("-")}_${product.category}`,
                },
                [
                  createCustomElement("div", { class: "product-item__content" }, [
                    createCustomElement("div", { class: "product-item__image" }),
                    createCustomElement("h3", { class: "product-item__title", textContent: product.title }),
                  ]),
                ]
              );
